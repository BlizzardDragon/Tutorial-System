...
namespace GameSystem
{
        ...
        IGameConstructElement
    {
        ...
        public virtual IGameElement GetElement(Type type)
        {
            return this.ScanElement(type);
        }
        ...
        private IGameElement ScanElement(Type type)
        {
            var fields = this.cachedFields;
            for (int i = 0, count = fields.Count; i < count; i++)
            {
                var field = fields[i];
                if (field.isElement && field.type == type)
                {
                    return field.value as IGameElement;
                }
            }
            
            throw new InvalidOperationException($"Element whith type {type} is not found!");
        }
